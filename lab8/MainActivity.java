...
public class MainActivity extends AppCompatActivity {
    ...
    int stateTransMatrix[][] = { // stateTransMatrix[currGameState][GameCommand] --> nextGameState
            { -1, 1, -1, -1, -1 },  // [Initial][Start] --> Running
            { 0, -1, 2, -1, 1 },    // [Running][Quit] --> Initial,
            // [Running][Pause] --> Paused,
            // [Running][Update] --> Running,
            { 0, 1, -1, 1, 2 },     // [Paused][Quit] --> Initial
            // [Paused][Start,Resume] --> Running
            // [Paused][Update] --> Paused
    };
    private void executeCommand(GameCommand cmd, char key) {
        gameState = GameState.stateFromInteger(stateTransMatrix[gameState.value()][cmd.value()]);
        if (gameState == GameState.Error)
            Log.d("MainActivity", "gameState is in error state!!!");
